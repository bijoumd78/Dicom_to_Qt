    ...
    Log_path( QDir::currentPath())
{
    ...
    connect(ui->pushButton_Browse, SIGNAL(clicked()),SLOT(FindFile()));
    connect(ui->Dicom_Annotation, SIGNAL(clicked()), SLOT(dicom_Annotation()));


    watcher = new QFileSystemWatcher(this);
    watcher->addPath(QDir::currentPath() + QString("/Test_Folder"));
...
void Dialog:: dicom_Annotation()
{
    ...
    if (!filename.isNull())
    {
        ...
        // std::string PatOrient = _PatOrient.substr(0,3); // First three characters
        ...
        */
        ...
        if (angle_needle_off_slice <= 60)   // draw a Needle Annotation as Linear Line
        {
            ...
            // Keep aspect ratio
            QImage imageQt_Scaled = imageQt->scaled(512, 512, Qt::KeepAspectRatio);

            // Write some Dicom Info on the display
            std::unique_ptr<QPainter> painter(new QPainter(&imageQt_Scaled));
            painter->setPen(Qt::green);
            painter->setFont(QFont("Time New Roman",8));

            //Draw the text on the image
            painter->drawText( QPoint(2,10),    QString::fromStdString(sf.ToStringPair(gdcm::Tag(0x0010,0x0010)).second));
            painter->drawText( QPoint(2,22),    QString::fromStdString("ID: " + sf.ToStringPair(gdcm::Tag(0x0010,0x0020)).second));
            painter->drawText( QPoint(2,34),    QString::fromStdString("FOV: " + std::to_string(FOVx) + "*" + std::to_string(FOVy)));

            painter->setPen(Qt::white);
            painter->setFont(QFont("Time New Roman",16));
            painter->drawText( QPoint(4,260),    QString::fromStdString(Letter_H));
            painter->drawText( QPoint(260,20),   QString::fromStdString(Letter_V));

            painter->setPen(Qt::green);
            painter->setFont(QFont("Time New Roman",8));
            painter->drawText( QPoint(320,10),  QString::fromStdString(sf.ToStringPair(gdcm::Tag(0x0008,0x0080)).first + ": "
                                                                       + sf.ToStringPair(gdcm::Tag(0x0008,0x0080)).second));
            painter->drawText( QPoint(320,22),  QString::fromStdString("Date: " + sf.ToStringPair(gdcm::Tag(0x0008,0x0023)).second));
            painter->drawText( QPoint(2,486),   QString::fromStdString("TR: " + sf.ToStringPair(gdcm::Tag(0x0018,0x0080)).second + " ms"));
            painter->drawText( QPoint(2,498),   QString::fromStdString("TE: " + sf.ToStringPair(gdcm::Tag(0x0018,0x0081)).second + " ms"));
            painter->drawText( QPoint(2,510),   QString::fromStdString("FA: " + sf.ToStringPair(gdcm::Tag(0x0018,0x1314)).second + " deg"));
            ...
            */
            ...
            {// create physical needle pattern
                ...
                */
                ...
                for(size_t ii = 0; ii < Npairs; ++ii){
                    if (2*ii+1 < Npts)
                        pointPairs << QPoint(std::ceil(pixel_locs_X_Y[2*ii].first * 512/800), std::ceil(pixel_locs_X_Y[2*ii+1].first * 512/800)) // scaling factor
                                << QPoint(std::ceil(pixel_locs_X_Y[2*ii].second * 512/800), std::ceil(pixel_locs_X_Y[2*ii+1].second * 512/800));
                }
                ...
                // Load the dicom file
                ui->label->setPixmap(QPixmap(QPixmap::fromImage(imageQt_Scaled)));
            ...
            if (virtual_max_distance > 0){//% create virtual needle pattern
                ...
                */
                ...
                for(size_t ii = 0; ii < Npairs; ++ii){
                    if (2*ii+1 < Npts)
                        pointPairs << QPoint(std::ceil(pixel_locs_X_Y[2*ii].first * 512/ImageNrows), std::ceil(pixel_locs_X_Y[2*ii+1].first * 512/ImageNrows)) // scaling factor
                                   << QPoint(std::ceil(pixel_locs_X_Y[2*ii].second * 512/ImageNcols), std::ceil(pixel_locs_X_Y[2*ii+1].second * 512/ImageNcols));
                }
                ...
                for (int ii = 0; ii < Npairs; ++ii){
                    if (colorperpair[ii] == 121){
                        ...
                        //line.setP1(QPointF(pointPairs[2*ii+1].x(),  pointPairs[2*ii+1].y()));
                        //line.setP2(QPointF(pointPairs[0].x(),  pointPairs[0].y()));
                    }
                    else if (colorperpair[ii] == 114){
                        QPen pen1(Qt::red, 4);
                        pen1.setStyle(Qt::DashLine);
                ...
                */
                ...
                {
                    int target_x =  std::ceil(psi_centralpix.first * 512/ImageNrows); // 512/ImageNrows is the scaling factor
                    int target_y =  std::ceil(psi_centralpix.second * 512/ImageNcols);
                    int target_r = 5;
                ...
                {
                    int target_x =  std::ceil(msi_centralpix.first * 512/ImageNrows); // 512/ImageNrows is the scaling factor
                    int target_y =  std::ceil(msi_centralpix.second * 512/ImageNcols);
                    int target_r = 5;
                ...
                // Show the tip of the Needle
                bool ShowNeedleTip= true;
                if (ShowNeedleTip){
                    int target_x =  std::ceil(tip_plot_pix.first * 512/ImageNrows); // 512/ImageNrows is the scaling factor
                    int target_y =  std::ceil(tip_plot_pix.second * 512/ImageNcols);
                    //int target_r = 5;
                    ...
                    painter->setPen(pen);
                    painter->drawPoint(QPointF(target_x, target_y));
            ...
            */
            ...
            if (D > Slice_Thick/2){
                ...
                target_x =  std::ceil(msi_centralpix.first * 512/ImageNrows); // 512/ImageNrows is the scaling factor
                target_y =  std::ceil(msi_centralpix.second * 512/ImageNcols);
...
void Dialog::FindFile()
{
    ...
    if (!filename.isNull())
    {
        ...
        std::cout <<  sf.ToStringPair(gdcm::Tag(0x0018,0x5100)).first << ": "
                   <<  sf.ToStringPair(gdcm::Tag(0x0018,0x5100)).second << std::endl;

        std::cout <<  sf.ToStringPair(gdcm::Tag(0x0020,0x0032)).first << ": "
                   <<  sf.ToStringPair(gdcm::Tag(0x0020,0x0032)).second << std::endl;

        std::cout <<  sf.ToStringPair(gdcm::Tag(0x0020,0x0037)).first << ": "
                   <<  sf.ToStringPair(gdcm::Tag(0x0020,0x0037)).second << std::endl;

        std::cout <<  sf.ToStringPair(gdcm::Tag(0x0018,0x0050)).first << ": "
                   <<  sf.ToStringPair(gdcm::Tag(0x0018,0x0050)).second << std::endl;

        std::cout <<  sf.ToStringPair(gdcm::Tag(0x0028,0x0030)).first << ": "
                   <<  sf.ToStringPair(gdcm::Tag(0x0028,0x0030)).second << std::endl;

        // Declare private tag that we need to find
        gdcm::PrivateTag pt1(0x51, 0x12,"SIEMENS MR HEADER" ); //Table position
        gdcm::DataSet &ds = file.GetDataSet();

        if( !ds.FindDataElement( pt1 ) )
            std::cout << "Table position private tag (0051,1012) not found ....." << std::endl;
        else
        {
        ...
        // std::cout <<  sf.ToStringPair(gdcm::Tag(0x0051,0x1012)).first << ": "
        //          <<  sf.ToStringPair(gdcm::Tag(0x0051,0x1012)).second << std::endl;

        std::cout <<  sf.ToStringPair(gdcm::Tag(0x0028,0x0010)).first << ": "
                   <<  sf.ToStringPair(gdcm::Tag(0x0028,0x0010)).second << std::endl;

        std::cout <<  sf.ToStringPair(gdcm::Tag(0x0028,0x0011)).first << ": "
                   <<  sf.ToStringPair(gdcm::Tag(0x0028,0x0011)).second << std::endl;
        ...
        // Get image orientation
        /*char */ auto Letter_H = getOrientation_H(orientation_matrix);
        /*char */ auto Letter_V = getOrientation_V(orientation_matrix);
        ...
        painter->drawLine(QPoint(156, 156), QPoint(456, 456));
#endif
...
void Dialog::directoryChanged(const QString &path)
{
    if(ui->checkBox_1->isChecked())
    {
        ...
        if (!list.isEmpty()){
            qDebug() << list.last().absoluteFilePath();
            FindFile_dropped(list.last().absoluteFilePath());
        }
    }
}

void Dialog::FindFile_dropped(QString filename)
{
    //QString filename = QFileDialog::getOpenFileName(this, tr("Open File"), QDir::currentPath());
    ...
    if (!filename.isNull())
    {
        ...
        // Get the largest image pixel value
        // uint LargestImagePixelValue = std::stoi(sf.ToStringPair(gdcm::Tag(0x0028,0x0107)).second);
        ...
        //Draw the text on the image
        painter->drawText( QPoint(2,10),    QString::fromStdString(sf.ToStringPair(gdcm::Tag(0x0051,0x100c)).second));
        painter->drawText( QPoint(2,22),    QString::fromStdString("Acq Matrix: " + sf.ToStringPair(gdcm::Tag(0x0051,0x100b)).second));
...
std::pair<double, double> Dialog::pix_3d_ptsfrom3d(double newlinept1xyz[], double newlinenorm[], double TLC_XYZmm[], double BRC_XYZmm[], double TRC_XYZmm[], double Im_rowdircoldir[], double thisPixSizeLocalYX[], double *newlineintersect_XYZmm)
{
...
std::tuple<double, double, double, double> Dialog::precise_phys_virt_na_dist(double thisFOVmm_LocalYX[], double thisPixSizeLocalYX[], double ImageNcols, double ImageNrows, double TLC_XYZmm[],
                                                                             double TRC_XYZmm[], double BRC_XYZmm[], double ImageNormal[], double Im_rowdircoldir[], double one_XYZ_pLPH[], double one_IJK_pLPH[])
{
...
bool Dialog::ConvertToFormat_RGB888(gdcm::Image const & gimage, char *buffer, QImage *&imageQt)
{
...
void Dialog::stringParser(std::string s, double *outputMatrix, size_t size)
{
    std::string token = "\\";
...
char *Dialog::getOrientation_H(double orientation_Matrix[])
{
...
char *Dialog::getOrientation_V(double orientation_Matrix[])
{
    char *orientation = new char[4];
    char *optr = orientation;
    *optr='\0';
